import { useChat, UseChatOptions, ToolCallHandler } from '@vercel/ai/react';
...
export function registerToolHandler(name: string, schema: ZodTypeAny, handler: (args: any) => Promise<any> | any) {
  registry.set(name, { schema, handler });
}

function ensureHighlightStyle() {
  if (document.getElementById('instrument-style')) return;
...
function highlightLinks({ path }: { path: string }) {
...
export function useInstrumentedChat(options: InstrumentedChatOptions = {}) {
  const onToolCall: ToolCallHandler = async (toolCall) => {
